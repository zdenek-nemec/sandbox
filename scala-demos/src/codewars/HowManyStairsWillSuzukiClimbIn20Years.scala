...
// https://www.codewars.com/kata/56fc55cd1f5a93d68a001d4e/train/scala
object HowManyStairsWillSuzukiClimbIn20Years {
  def stairsIn20(stairs: Seq[Seq[Int]]): Int = {
    var stairs_total = 0
    for (day <- stairs) {
      for (stairs_in_day <- day) {
        stairs_total = stairs_total + stairs_in_day
      }
    }
    stairs_total * 20
  }
  ...
  // Much better
  def stairsIn20ByAlexiush(stairs: Seq[Seq[Int]]): Int = stairs.flatten.sum * 20

  def main(args: Array[String]): Unit = {
    println("How many stairs will Suzuki climb in 20 years?")
    ...
    val stairs = Seq(sunday, monday, tuesday, wednesday, thursday, friday, saturday)

    println(stairsIn20(stairs))
    println(stairsIn20ByAlexiush(stairs))
  }
}
